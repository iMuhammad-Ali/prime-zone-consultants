...
export default function AutoLoopSlider({
  speed = 2, // Speed in pixels per second
  children,
  className = "",
}: AutoLoopSliderProps) {
  ...
  return (
    ...
    >
      ...
      {/* Scrolling Content */}
      <motion.div
        className="absolute whitespace-nowrap flex items-center gap-[8vw] sm:gap-[6vw] lg:gap-[4vw] xl:gap-[3vw] 2xl:gap-[2.5vw] h-full pl-[20vw] sm:pl-[15vw] lg:pl-[12vw] xl:pl-[10vw] 2xl:pl-[8vw] text-[3.5vw] sm:text-[2.5vw] lg:text-[1.4vw] xl:text-[1.1vw] 2xl:text-[0.9vw] text-foreground/80 font-medium"
        style={{ x }}
      >
          ...
          >
            {React.Children.map(children, (child, index) => (
              <React.Fragment key={`${copyIndex}-${index}`}>
                {index > 0 && (
                  <span className="text-primary text-[2vw] sm:text-[1.5vw] lg:text-[1vw] xl:text-[0.8vw] 2xl:text-[0.6vw]">
                    •
                  </span>
                )}
